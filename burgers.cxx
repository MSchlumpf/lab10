...
int main(){
  ...
  double* u0 = new double[N];	//u von n-1
  double* u1 = new double[N];	//u von n
  double* u2 = new double[N];	//u von n+1
  double* h;
  stringstream strm;

  initialize(u1,u0,dx,dt, xmin,N);

  writeToFile(u1, "u_0", dx, xmin, N, t);
  ...
  for(int i=1; i<=Na; i++)
  {
   ...
   strm.str("");
   strm << "u_" << i;
   writeToFile(u1, strm.str(), dx, xmin, N, t);
  }
                ...
                const double dt, const double xmin,  const int N)
{
   ...
   for(int i=0; i<N; i++)
   {
     ...
     u1[i] = u;
     u0[i] = u - dt* (-u)*ux + 1/2*dt*dt * (2*u*pow(ux,2) + pow(u,2) * uxx);
   }
}
//-----------------------------------------------
void writeToFile(const double* const u, const string s, const double dx,
